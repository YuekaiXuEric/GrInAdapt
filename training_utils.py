...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, reduction='mean', weight=None):
        """
        :param alpha: balancing factor for classes. It can be a scalar or a list/tuple of per-class weights.
        :param gamma: focusing parameter to down-weight easy examples.
        :param reduction: 'mean', 'sum', or 'none'
        :param weight: Optional tensor of shape (C,) specifying a manual rescaling weight for each class.
                       This overrides alpha if provided.
        """
        super(FocalLoss, self).__init__()
        if isinstance(alpha, (list, tuple)):
            # Convert list/tuple to tensor and register as buffer so that it's moved along with the model.
            self.register_buffer('alpha', torch.tensor(alpha))
        else:
            self.alpha = alpha  # scalar
        self.gamma = gamma
        self.reduction = reduction
        self.weight = weight  # per-class weight tensor if provided
    ...
    def forward(self, inputs, targets):
        ...
        # Gather the log-probabilities and probabilities for the targets
