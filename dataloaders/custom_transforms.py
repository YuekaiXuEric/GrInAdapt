...
def to_multilabel(pre_mask, classes=2):  # notice classes para
...
class add_salt_pepper_noise_BraTS():
    def __call__(self, sample):


        images_0 = np.array(sample['image'][0]).astype(np.uint8)
        images_1 = np.array(sample['image'][1]).astype(np.uint8)
        images_2 = np.array(sample['image'][2]).astype(np.uint8)
        images_3 = np.array(sample['image'][3]).astype(np.uint8)
        X_imgs_copy = np.zeros([images_0.shape[0], images_0.shape[0], 4], dtype=np.uint8)
        X_imgs_copy[:, :, 0] = images_0.copy()
        X_imgs_copy[:, :, 1] = images_1.copy()
        X_imgs_copy[:, :, 2] = images_2.copy()
        X_imgs_copy[:, :, 3] = images_3.copy()
        # row = image.shape[0]
        # col = image.shape[1]
        salt_vs_pepper = 0.2
        amount = 0.004
...
class RandomCrop_BraTS(object):
    ...
    def __call__(self, sample):
        imgs, mask = sample['image'], sample['label']
        w, h = imgs[0].size
        if self.padding > 0 or w < self.size[0] or h < self.size[1]:
            padding = np.maximum(self.padding,np.maximum((self.size[0]-w)//2+5,(self.size[1]-h)//2+5))
            imgs[0] = ImageOps.expand(imgs[0], border=padding, fill=0)
            imgs[1] = ImageOps.expand(imgs[1], border=padding, fill=0)
            imgs[2] = ImageOps.expand(imgs[2], border=padding, fill=0)
            imgs[3] = ImageOps.expand(imgs[3], border=padding, fill=0)
            mask = ImageOps.expand(mask, border=padding, fill=255)

        assert imgs[0].width == mask.width
        assert imgs[0].height == mask.height
        w, h = imgs[0].size
        th, tw = self.size # target size
        if w == tw and h == th:
            return {'image': imgs,
        ...
        x1 = random.randint(0, w - tw)
        y1 = random.randint(0, h - th)
        imgs[0] = imgs[0].crop((x1, y1, x1 + tw, y1 + th))
        imgs[1] = imgs[1].crop((x1, y1, x1 + tw, y1 + th))
        imgs[2] = imgs[2].crop((x1, y1, x1 + tw, y1 + th))
        imgs[3] = imgs[3].crop((x1, y1, x1 + tw, y1 + th))
        mask = mask.crop((x1, y1, x1 + tw, y1 + th))
        return {'image': imgs,
                'label': mask,
                'img_name': sample['img_name']}
...
class RandomFlip_BraTS(object):
    def __call__(self, sample):
        ...
        if random.random() < 0.5:
            ...
            imgs[1] = imgs[1].transpose(Image.FLIP_LEFT_RIGHT)
            imgs[2] = imgs[2].transpose(Image.FLIP_LEFT_RIGHT)
            imgs[3] = imgs[3].transpose(Image.FLIP_LEFT_RIGHT)
            mask = mask.transpose(Image.FLIP_LEFT_RIGHT)
        if random.random() < 0.5:
            imgs[0] = imgs[0].transpose(Image.FLIP_TOP_BOTTOM)
            imgs[1] = imgs[1].transpose(Image.FLIP_TOP_BOTTOM)
            imgs[2] = imgs[2].transpose(Image.FLIP_TOP_BOTTOM)
            imgs[3] = imgs[3].transpose(Image.FLIP_TOP_BOTTOM)
            mask = mask.transpose(Image.FLIP_TOP_BOTTOM)
...
class RandomRotate_BraTS(object):
    def __init__(self, size=512):
        self.degree = random.randint(1, 4) * 90
        self.size = size
    ...
    def __call__(self, sample):
        ...
        if seed > 0.5:
            rotate_degree = self.degree
            imgs[0] = imgs[0].rotate(rotate_degree, Image.BILINEAR, expand=0)
            imgs[1] = imgs[1].rotate(rotate_degree, Image.BILINEAR, expand=0)
            imgs[2] = imgs[2].rotate(rotate_degree, Image.BILINEAR, expand=0)
            imgs[3] = imgs[3].rotate(rotate_degree, Image.BILINEAR, expand=0)
            mask = mask.rotate(rotate_degree, Image.NEAREST, expand=255)

            sample = {'image': imgs, 'label': mask, 'img_name': sample['img_name']}
        return sample
...
class RandomScaleCrop_BraTS(object):
    def __init__(self, size):
        self.size = size
        self.crop = RandomCrop_BraTS(self.size)
    ...
    def __call__(self, sample):
        ...
        seed = random.random()
        if seed > 0.5:
            w = int(random.uniform(0.5, 1.5) * imgs[0].size[0])
            h = int(random.uniform(0.5, 1.5) * imgs[0].size[1])

            imgs[0] = imgs[0].resize((w, h), Image.BILINEAR)
            imgs[1] = imgs[1].resize((w, h), Image.BILINEAR)
            imgs[2] = imgs[2].resize((w, h), Image.BILINEAR)
            imgs[3] = imgs[3].resize((w, h), Image.BILINEAR)
            mask = mask.resize((w, h), Image.NEAREST)
            sample = {'image': imgs, 'label': mask, 'img_name': name}
...
class Resize_BraTS(object):
    ...
    def __call__(self, sample):
        ...
        imgs[0] = imgs[0].resize((self.size, self.size), Image.BILINEAR)
        imgs[1] = imgs[1].resize((self.size, self.size), Image.BILINEAR)
        imgs[2] = imgs[2].resize((self.size, self.size), Image.BILINEAR)
        imgs[3] = imgs[3].resize((self.size, self.size), Image.BILINEAR)
        mask = mask.resize((self.size, self.size), Image.NEAREST)
...
class Normalize_BraTS(object):
    """
    Normalize a tensor image.
    """
    def __call__(self, sample):
        if type(sample['image']) == list:
            img0 = np.array(sample['image'][0]).astype(np.float32)
            img1 = np.array(sample['image'][1]).astype(np.float32)
            img2 = np.array(sample['image'][2]).astype(np.float32)
            img3 = np.array(sample['image'][3]).astype(np.float32)
            img = np.zeros((img0.shape[0], img0.shape[1], 4), dtype=np.float32)
            img[:, :, 0] = img0
            img[:, :, 1] = img1
            img[:, :, 2] = img2
            img[:, :, 3] = img3
...
class LabelOneHotAndToTensor(object):
    """
    Turn number(0, 80, 160, 180) label into one-hot label.
    Convert PIL.Image in sample to Tensors.
    """
    def __call__(self, sample):
        img = np.array(sample['image']).astype(np.float32).transpose((2, 0, 1))
        img = torch.from_numpy(img).type(dtype=torch.FloatTensor)

        map = np.array(sample['label'])
        map = (map == 80) * 1 + (map == 160) * 2 + (map == 240) * 3
        map = torch.from_numpy(map).type(dtype=torch.LongTensor)
        label_onehot = torch.FloatTensor(4, map.size()[0], map.size()[1])
        label_onehot.zero_()
        label_onehot.scatter_(0, map.unsqueeze(dim=0), 1)
