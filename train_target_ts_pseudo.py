...
def colorize_segmentation_faz(seg_idx):
    """
    Convert a single-channel integer segmentation map (H,W)
    into an RGB color image (H,W,3).
...
def save_segmentation_png_train(merge_pseudo_label, gt_seg, pred_seg, pred_seg_s, map_s, sample, save_dir='.', prefix='sample', mode='train'):
    ...
    for i in range(N):
        ...
        merge_pseudo_label_img = colorize_segmentation_faz(merge_pseudo_label[i])
        ...
        axes[i, 4].set_title("proto_psuedo_label")
        axes[i, 4].axis('off')
...
def adapt_epoch(args, model_t, model_s, optim, train_loader, test_loader, start_step, current_epoch):
    ...
    for sample_w, sample_s in tqdm(train_loader, total=len(train_loader), desc="Adapting"):
        ...
        merge_pseudo_label = sample_w['proto_pseudo_npz']
        ...
        for i in range(anatomical_labels.shape[0]):
            if args.mask_optic_disc and anatomical_labels[i].item() == 1:
                merge_pseudo_label[i] = torch.where(merge_pseudo_label[i] == 1,
        ...
        teacher_weights = torch.tensor([1.0, 1.0, 1.0, 1.0, 0.8], device='cuda')
        ...
        fg_logits = cavf_predictions_stu_s[:, 4:5, ...]
        bg_logits = cavf_predictions_stu_s[:, :4, ...].mean(dim=1, keepdim=True)
        ...
        merge_weights = torch.tensor([1.0, 1.1], device='cuda')

        Loss_CE_merge = torch.nn.CrossEntropyLoss(weight=merge_weights)
        Loss_DSC_merge = DiceLoss(weight=merge_weights)
        seg_loss_merge = Loss_CE_merge(out_2ch, merge_pseudo_label) + Loss_DSC_merge(out_2ch, merge_pseudo_label)

        seg_loss = seg_loss_teacher + 1.7*seg_loss_merge
